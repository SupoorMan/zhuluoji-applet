    ...
    <view class="detail-card">
      <view class="" style="position: relative; top: -24rpx">
        <van-divider contentPosition="center" dashed
          >请填写客观真实的评价</van-divider
        >
      </view>
      <view class="eval-tags">
        <view
          v-for="tag in recomdTag"
          :key="tag"
          @tap="checkTag(tag)"
          :class="{ active: activeTag.includes(tag) }"
          >{{ tag }}</view
        >
      </view>
      <view class="detail-content">
        <view class="eval-area">
          ...
          <textarea
            ...
            cols="30"
            rows="6"
            placeholder="从多个角度评价产品，可以帮助更多想了解的人~"
            @input="inputChange"
          ></textarea>
        </view>
        <van-uploader
          @after-read="afterRead"
          image-fit="scaleToFit"
          max-count="5"
          :file-list="fileList"
          @delete="handleDelete"
        >
        ...
        <view class="no-name-box">
          <van-checkbox :value="noName" @change="onChange" icon-size="32rpx"
            >匿名
          </van-checkbox>
