<template>
  <view>
    <van-cell
      center="true"
      value-class="time-text"
      use-label-slot
      :value="cutDate(item.createTime)"
    >
      <template #icon>
        <van-image
          round
      ...
      <template #title>
        ...
        <text style="color: #edba00; font-size: 24rpx"
    ...
    <view class="eval-message" :style="noreply ? 'padding-right:0;' : ''">
      ...
      <div class="review-tools">
        <view @click="replyToEval(item)" v-if="!noreply">
          <van-icon name="chat-o" size="32rpx" style="margin-right: 4rpx" />
          回复
        </view>
        <view @click="addStar">
          <van-icon
            :name="item.flag === 1 ? 'like' : 'like-o'"
            size="32rpx"
            color="#FF4B4B"
