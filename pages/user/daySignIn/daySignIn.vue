		...
		<view class="">
			<view class="top-inter">
				<view class="inter-content">
					<text class="inter-text">{{user.integral||0}}</text>
					<van-icon name="/static/shop/inter-icon.png" size="40rpx" />
				</view>
				<navigator class="inter-btn" url="/pages/shop/index">
					<text>去兑换</text>
					<van-icon name="arrow" />
				</navigator>
			</view>
			<view class="calendar-content">
				...
				<view class="calendar-body">
					...
					<van-calendar readonly :show-confirm="false" :show-title="false" :poppable="false"
						...
						:default-date="signDays" class="calendar" @confirm="onConfirm" />
			...
			<view class="sign-tip">
				<van-row custom-class="tip-title">
					<van-col span="8">签到奖励</van-col>
		...
		methods: {
			...
			async getSigns() {
				...
				data.sign.split('').forEach((isSign, index) => {
					if (isSign === '1') {
						signDays.push(new Date(monthDays[index]))
					}
				})
